---
title: "Students"
layout: single
permalink: /students/
author_profile: true
toc: true
---

# Current Students

<details open markdown="1">
<summary><strong>Ph.D. Students</strong></summary>
<div markdown="1">
  - Mohammad Taha Askari  
    - UBC 4YF Scholar
  - Elena Dobre
  - Nikhileswar Kota
  - Idan Roth
  - Selvakumar Tharranetharan  
    - UBC 4YF Scholar
</div>
</details>

<details open>
<summary><strong>M.A.Sc. Students</strong></summary>

- Chinmoyee Chakraborty

- Francis Chambers

- Flynn Dowey

</details>

---
## Ph.D. Students
<details>
<summary><strong>Show Ph.D. Graduates</strong></summary>

- Faramarz Jabbarvaziri (Ph.D.)
  - Thesis: [Advanced Grant-Free NOMA and Hybrid Beamforming Techniques for 5G and Beyond](https://open.library.ubc.ca/soa/cIRcle/collections/ubctheses/24/items/1.0448441)

- Mostafa Darabi (Ph.D.)
  - Thesis: [Resource allocation for eMBB and URLLC use cases in 5G and beyond cellular networks](https://open.library.ubc.ca/soa/cIRcle/collections/ubctheses/24/items/1.0445155)

- Yinjia Huo (Ph.D.)
  - Thesis: [Cable health monitoring using power line communications signal and machine learning technique](https://open.library.ubc.ca/soa/cIRcle/collections/ubctheses/24/items/1.0437183)

- Vishnu Rajendran Chandrika (Ph.D.)
  - Thesis: [Enhancing low-cost cellular IoT networks on the sidelink and satellite links](https://open.library.ubc.ca/soa/cIRcle/collections/ubctheses/24/items/1.0422778)

- Rabe Arshad (Ph.D.)
  - Thesis: [Stochastic geometry aided user mobility analysis in heterogeneous wireless networks](https://open.library.ubc.ca/soa/cIRcle/collections/ubctheses/24/items/1.0421061)

- Ali Saberali (Ph.D.)
  - Thesis: [Coded caching: convex optimization and graph theoretical perspectives](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0394742)

- Yanan Sun (Ph.D.)
  - Thesis: [Application of Behind-the-Meter Energy Storage Systems for Household Load Hiding and Frequency Regulation Service](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0394046)

- Mrinmoy Jana (Ph.D.)
  - Thesis: [Faster-than-Nyquist system design for next generation fixed transmission networks](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0385498)

- Gautham Prasad (Ph.D.)
  - Thesis: [In-Band Full-Duplex Power Line Communications](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0377128)

- Faribar Aalamifar (Ph.D.)
  - Thesis: [Analysis and Optimization of Communication Technologies for Smart Grid Applications](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0368720)

- Md. Jahidur Rahman (Ph.D.)
  - Thesis: [Interference mitigation and alignment for interference-limited communication systems](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0362403)

- Hao Ma (Ph.D.)
  - Thesis: [Coordinated Transmission for Visible Light Communication Systems](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0357180)

- Naveen Mysore Balasubramanya (Ph.D.)
  - Thesis: [Enhancements to the Long Term Evolution (LTE) Standard for Facilitating the Internet of Things (IoT)](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0347534)

- Ayman Mostafa (Ph.D.)
  - Thesis: [Physical-Layer Security for Visible-Light Communication Systems](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0345607)

- Julien Renard (Ph.D.)
  - Thesis: [Spectrum Sensing for Cognitive Radios: Improving Robustness to Impulsive Noise](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0305018)

- Nikola Zlatanov (Ph.D.)
  - with Prof. Horlin, jointly at UBC and Université Libre de Bruxelles
  - Thesis: [Novel Protocols for the Two-Hop Half-Duplex Relay Network](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0166356)

- Nima Taherinejad (Ph.D.)
  - main supervisor Prof. R. Schober
  - Thesis: [Power line communications in vehicles: Channel measurements and impedance matching networks](https://circle.ubc.ca/handle/2429/52749)

- Imtiaz Ahmed (Ph.D.)
  - co-supervised by Prof. S. Mirabbasi
  - Thesis: [Resource Allocation in Wireless Systems with Conventional and Energy Harvesting Nodes](https://circle.ubc.ca/handle/2429/50857)

- Ghasem Naddafzadeh Shirazi (Ph.D.)
  - main supervisor Prof. R. Schober
  - Thesis: [Optimization in Wireless Sensor and Machine-Type Communication Networks](https://circle.ubc.ca/handle/2429/46658)

- Zahra Ahmadian (Ph.D.)
  - Thesis: [Multiuser Pre-Filtered Ultra-Wideband Systems](https://circle.ubc.ca/handle/2429/45592)

- Roee Diamant (Ph.D.)
  - Thesis: [Spatial Reuse Scheduling and Localization for Underwater Acoustic Communication Networks](https://circle.ubc.ca/handle/2429/44893)

- Azad Ravanshid (Ph.D.)
  - Thesis: [Rateless-Coded Cooperative Communications for Wireless Networks](http://opus4.kobv.de/opus4-fau/frontdoor/index/index/docId/3530)

- Mohammad Mohammadnia-Avval (Ph.D.)
  - at University of Erlangen-Nurnberg, with Prof. J. Huber
  - Thesis: [Analysis and design of OFDM systems: Loading and nonlinear distortion mitigation](https://circle.ubc.ca/handle/2429/37610)

- Trung Thanh Nguyen (Ph.D.)
  - Thesis: [Binary-Coded Modulation and Applications in Free-Space Optics](https://circle.ubc.ca/handle/2429/33145)

- Jeebak Mitra (Ph.D.)
  - Thesis: [Reliable communication in non-Gaussian environments: Receiver design and analytical aspects](http://hdl.handle.net/2429/24695)

- Tariq Al-Khasib (Ph.D.)
  - Thesis: [Resource allocation and optimization for multiple-user legacy and cognitive radio systems](http://hdl.handle.net/2429/24588)

- Anand Oka (Ph.D.)
  - co-supervised by Prof. H. Alnuweiri
  - Thesis: [Towards Vaulting the Hurdle of Short Lifetimes in Wireless Sensor Networks: Distributed Algorithms and UWB Impulse Radio](http://hdl.handle.net/2429/11080)

- Anna-Marie Silvester (Ph.D.)
  - Thesis: [Space-Time Continuous-Phase Modulation](Preprints/Anna_Maries_Thesis.pdf)

- Christopher Snow (Ph.D.)
  - Thesis: [Multiband OFDM for Ultra-Wideband Wireless Communication](https://circle.ubc.ca/dspace/handle/2429/318)

- Volker Pauli (Ph.D.)
  - co-supervised with Prof. R. Schober
  - at University of Erlangen-Nurnberg, with Prof. J. Huber
  - Thesis: [Design and Analysis of Low-Complexity Noncoherent Detection Schemes](Preprints/PhDThesis_VolkerPauli.pdf)

</details>

---
## MASc Students
<details>
<summary><strong>Show MASc Graduates</strong></summary>
- Idan Roth (M.A.Sc.)
  - Thesis: Learning to Calibrate: Graph Neural Network for Online Calibration of Phase Shifters in Hybrid Beamforming Systems

- Marley Beckett (M.A.Sc.)
  - Thesis: Harnessing RF Energy for Sustainable Power in Ambient IoT Devices

- Xi Lu (M.A.Sc.)
  - Thesis: [Neural networks in application to dolphin whistle detection and generation](https://open.library.ubc.ca/soa/cIRcle/collections/ubctheses/24/items/1.0438552)

- Mitchell Nichols (M.A.Sc.)
  - Thesis: [Fully connected optical beamforming network using microring weight banks](https://open.library.ubc.ca/soa/cIRcle/collections/ubctheses/24/items/1.0431626)

- Mehdi Saeidi (M.A.Sc.)
  - Thesis: [Hardware implementation of a multicarrier faster-than-Nyquist decoder](https://open.library.ubc.ca/soa/cIRcle/collections/ubctheses/24/items/1.0422011)

- Shenghang Luo (M.A.Sc.)
  - Thesis: [Model based machine learning techniques for fiber nonlinearity compensation](https://open.library.ubc.ca/soa/cIRcle/collections/ubctheses/24/items/1.0420692)

- Mohammad Taha Askari (M.A.Sc.)
  - Thesis: [Interplay between Fiber Nonlinearity and Probabilistic Amplitude Shaping](https://open.library.ubc.ca/soa/cIRcle/collections/ubctheses/24/items/1.0418426)

- Prasham Jain (M.A.Sc.)
  - Thesis: [Machine Learning Aided Non-Linear Interference Compensation for Optical Fiber Communication Systems](https://open.library.ubc.ca/soa/cIRcle/collections/ubctheses/24/items/1.0415785)

- Seyedrazieh Bayati (M.A.Sc.)
  - Thesis: [Machine learning-assisted CRAN design with hybrid RF/FSO and full-duplex self-backhauling](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0395114)

- Jordan Naterer (M.A.Sc.)
  - Thesis: [Modular spectrum utilization for next-generation fixed transmission networks](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0392708)

- Lazar Atanackovic (M.A.Sc.)
  - Thesis: [Machine learning inspired ship-radiated noise modelling and cancellation for underwater acoustic communication systems](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0392501)

- Roee Bar (M.A.Sc.)
  - Thesis: [In-Vehicle Powerline Communication Using Software-Defined Radio](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0307515)

- Manyou Ma (M.A.Sc.)
  - Thesis: [A Comparative Evaluation of Two Synthetic Aperture with Virtual Source Beamforming Methods in Biomedical Ultrasound](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0223123)

- Michelle Clark (M.A.Sc.)
  - co-supervised with Prof. R. Rohling
  - Thesis: [Improving the Feasibility of Energy Disaggregation in Very High- and Low-Rate Sampling Scenarios](https://circle.ubc.ca/handle/2429/54873)

- Philip Mark (M.A.Sc.)
  - Thesis: [Ethernet over Light](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0167659)

- Yifei Zhuang (M.A.Sc.)
  - Thesis: [Physical Layer Security in MIMO Power Line Communication Networks](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0166958)

- Mohamed Ahmed (M.A.Sc.)
  - Thesis: [Inference and Communications over Power Lines](https://circle.ubc.ca/handle/2429/44954)

- Kyle Kotowick (M.A.Sc.)
  - co-supervised with Prof. R. Rohling
  - Thesis: [Advances in Synthetic Aperture, Compounded Plane Wave, and Spatially Encoded Excitation Techniques for Fast Ultrasonography](https://open.library.ubc.ca/cIRcle/collections/ubctheses/24/items/1.0074121)

- Dana Hoffmann (M.A.Sc.)
  - Thesis: [Multiuser Detection for Transmit-Only Active RFID](https://circle.ubc.ca/handle/2429/39892)

- Xiaolang Zhang (M.A.Sc.)
  - Thesis: [A Programmable-Gain Amplifier and an Active Inductor for In-Vehicle Power Line Communications](https://circle.ubc.ca/handle/2429/37014)

- Kung-Chung Lee (M.A.Sc.)
  - co-supervised with Prof. S. Mirabbasi
  - Thesis: [Localization Systems Using Signal Strength Fingerprinting](https://circle.ubc.ca/handle/2429/28750)

- Alireza Kenarsari (M.A.Sc.)
  - Thesis: Rehashing Bit-Interleaved Coded Modulation

- Ali Abdul-Hussein (M.A.Sc.)
  - Thesis: Rateless Codes – Decoding and Applications

- Jingjun Li (M.A.Sc.)
  - Thesis: Detection, Channel Estimation, and Interference Suppression for DS-UWB

- Zahra Ahmadian (M.A.Sc.)
  - co-supervised by Prof. C. Leung
  - Thesis: Performance Analysis for IEEE 802.15.4a UWB Systems

- Trung Thanh Nguyen (M.A.Sc.)
  - Thesis: PAR Reduction in OFDM Systems by Multiple Signal Representation

- Siddharth Srinivasan (M.A.Sc.)
  - Thesis: Gallager's Random Coding Exponent for Differential Space-Time Modulation

- Jeebak Mitra (M.A.Sc.)
  - Thesis: Serial Concatenation of Simple Codes with Differential Modulations

- Noha Ibrahim (M.A.Sc.)
  - Thesis: Bluetooth Receiver Design Based on Laurent's Decomposition

- Ambuj Parihar (M.A.Sc.)
  - co-supervised with Prof. R. Schober
  - Thesis: Equalization for DS-UWB Systems

- Jeff Liu (M.A.Sc.)
  - co-supervised by Prof. C. Leung
  - Thesis: Low-Complexity Iterative Decoding for Bit-Interleaved Coded Modulation

- Mani Jain (M.A.Sc.)
  - co-supervised with Prof. R. Schober
  - Thesis: Noncoherent Sequence Detection Receiver for Bluetooth Systems

</details>
